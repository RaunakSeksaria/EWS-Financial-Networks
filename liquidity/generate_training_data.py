...
# Load the sibling model module by path so this script runs from any CWD.
script_dir = Path(__file__).parent
spec = importlib.util.spec_from_file_location("liquid_model", script_dir / "liquid_model.py")
...
def run_single_simulation(
    sim_id: int, params: LiquidityParams, A: np.ndarray, seed: int, verbose: bool = False
) -> dict:
    """
...
def generate_training_dataset(config: DataGenConfig, verbose: bool = True):
    ...
    while accepted_count < config.n_simulations:
        ...
        result = run_single_simulation(
            sim_id=sim_count, params=params, A=A, seed=sim_count * 42, verbose=False
        )
        ...
        pbar.update(1)
        pbar.set_postfix(
            {"accepted": accepted_count, "rejected": rejected_count, "windows": total_windows}
        )
...
def create_diagnostic_plots(
    metadata_df: pd.DataFrame, gc_drops: list, kappa_c_values: list, save_dir: Path
):
    ...
    axes[1, 2].pie(
        method_counts.values, labels=method_counts.index, autopct="%1.1f%%", startangle=90
    )
...
if __name__ == "__main__":
    ...
    print("✓ Load windows with: np.load('training_data/windows/<window_id>.npz')")
    print("✓ Load metadata with: pd.read_csv('training_data/metadata.csv')")
