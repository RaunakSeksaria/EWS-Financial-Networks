...
def run_comparison(params, seed=42):
    ...
    results["full_original"] = {"t": sol1.t, "x": sol1.y.T, "A": A_original}
    ...
    results["no_coupling"] = {"t": sol2.t, "x": sol2.y.T, "A": A_original}
    ...
    results["no_contagion"] = {"t": sol4.t, "x": sol4.y.T, "A": A_original}
    ...
    results["full_random"] = {"t": sol3.t, "x": sol3.y.T, "A": A_random}
...
def print_analysis(metrics):
    ...
    print(
        f"   No coupling (ε=0):                  κc = {no_coup['kappa_c']:.4f}"
        if no_coup["kappa_c"]
        else "   No coupling: No transition detected"
    )
