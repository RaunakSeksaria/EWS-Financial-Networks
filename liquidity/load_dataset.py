...
def load_dataset(data_dir: str = "training_data") -> tuple[pd.DataFrame, Path]:
    ...
    print(
        f"Loaded dataset with {len(metadata)} windows from {len(metadata['sim_id'].unique())} simulations"
    )
    return metadata, windows_path


def get_window(
    metadata: pd.DataFrame, windows_path: Path, idx: int
        ...
        with zipfile.ZipFile(windows_path, "r") as zip_ref:
            ...
            if found_path is None:
                raise FileNotFoundError(
                    f"Window {filename} not found in {windows_path}. Available: {zip_ref.namelist()[:5]}..."
                )
...
def get_simulation_windows(
    metadata: pd.DataFrame, windows_path: Path, sim_id: str
...
def dataset_summary(metadata: pd.DataFrame):
    ...
    print("\n--- Labels ---")
    print(
        f"κ_c range: [{metadata['kappa_c_label'].min():.4f}, {metadata['kappa_c_label'].max():.4f}]"
    )
    print(
        f"κ_c mean ± std: {metadata['kappa_c_label'].mean():.4f} ± {metadata['kappa_c_label'].std():.4f}"
    )
