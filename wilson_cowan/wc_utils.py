...
import pandas as pd
import torch
from config import EPSILON_MAX, EPSILON_MIN
from wc_dataset import collate_fn_graphs, create_windows_from_universes
...
def plot_figure_4b(model, test_universe, window_size, device):
    ...
    ax.scatter(
        eps_s_values,
        mean_activity[: len(predictions)],
        color="red",
        s=10,
        zorder=5,
        label=r"Prediction Start ($\epsilon_s$)",
    )
    ...
    for i in range(len(predictions)):
        ...
        ax.plot(
            [eps_s, eps_pred], [activity_s, activity_s], color=color, linestyle="-", linewidth=0.5
        )
        ax.plot(
            [eps_pred, eps_pred], [activity_s - 0.5, activity_s + 0.5], color=color, linewidth=0.5
        )
