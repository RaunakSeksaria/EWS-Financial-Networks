...
from config import WINDOW_SIZE, RNG_SEED
...
def get_data_loaders(universes, batch_size, seed):
    ...
    print(f"Splitting universes: {len(train_universes)} train, {len(val_universes)} val, {len(test_universes)} test")

    # Create windowed datasets for each split
    train_windows, train_ei, train_labels, _ = create_windows_from_universes(train_universes, WINDOW_SIZE)
    val_windows, val_ei, val_labels, _ = create_windows_from_universes(val_universes, WINDOW_SIZE)
    test_windows, test_ei, test_labels, test_lead_dist = create_windows_from_universes(test_universes, WINDOW_SIZE)
    
    print(f"Total windows: {len(train_windows)} train, {len(val_windows)} val, {len(test_windows)} test")
    ...
    # Create DataLoaders
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_graphs)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_graphs)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_graphs)
