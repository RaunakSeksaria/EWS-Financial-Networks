...
class FinancialNetworkSimulator:
    ...
    def __init__(self, n_firms: int = 100, n_banks: int = 10, 
                 topology: str = 'barabasi_albert', seed: int = 42):
        """
    ...
    def build_network(self, m: int = 3, p: float = 0.1, k: int = 4, beta: float = 0.3):
        ...
        for u, v in G_firms.edges():
            firm_u = f"F{u}"
            firm_v = f"F{v}"
            self.G.add_edge(firm_u, firm_v,
                           edge_type='contagion',
                           A=np.random.uniform(0.01, 0.1))  # contagion weight
        ...
        for j in range(self.n_banks):
            ...
            for i in connected_firms:
                ...
                self.G.add_edge(bank_id, firm_id,
                               edge_type='loan',
                               L=L_initial,           # loan amount
                               nu=0.0,                # allocation share (normalized later)
                               rho=np.random.uniform(0.3, 0.7))  # recovery fraction
        ...
        print(f"Firm-firm edges: {sum(1 for u, v, d in self.G.edges(data=True) if d['edge_type']=='contagion')}")
        print(f"Bank-firm edges: {sum(1 for u, v, d in self.G.edges(data=True) if d['edge_type']=='loan')}")
    ...
    def _normalize_allocation_shares(self):
        ...
        for firm_id in firm_nodes:
            ...
            lending_banks = [u for u, v, d in self.G.in_edges(firm_id, data=True) 
                           if d['edge_type'] == 'loan']
    ...
    def compute_firm_quantities(self, firm_id: str) -> Dict[str, float]:
        ...
        total_loans = sum(self.G[bank_id][firm_id]['L'] 
    ...
    def step(self, dt: float = 0.01):
        ...
        # === 1. Update firms ===
        firm_nodes = [n for n, d in self.G.nodes(data=True) 
                     if d['node_type'] == 'firm' and not d['defaulted']]
        ...
        for firm_id in firm_nodes:
            ...
            repayment_inflow = sum(self.G[bank_id][firm_id]['L'] / tau_L_f
                                  for bank_id in self.G.predecessors(firm_id)
                                  if self.G[bank_id][firm_id]['edge_type'] == 'loan')
            ...
            for bank_id in self.G.predecessors(firm_id):
                ...
                edge_updates[(bank_id, firm_id)] = {
                    'L': L_bf + dL
                }
        
        # === 2. Update banks ===
        bank_nodes = [n for n, d in self.G.nodes(data=True) 
                     if d['node_type'] == 'bank' and not d['defaulted']]
    ...
    def run_simulation(self, T: float = 100.0, dt: float = 0.01, 
                      record_interval: int = 10) -> pd.DataFrame:
        """
    ...
    def save_timeseries(self, df: pd.DataFrame, run_id: str, 
...
def generate_ensemble(n_runs: int = 10, 
                     n_firms: int = 100,
                     n_banks: int = 10,
                     T: float = 100.0,
                     dt: float = 0.01,
...
def build_feature_dataset(input_dir: str = 'simulation_runs',
...
if __name__ == "__main__":
    ...
    print(f"\nNetwork statistics:")
    print(f"Average firm degree: {np.mean([d for n, d in sim.G.degree() if n.startswith('F')]):.2f}")
